...
void Enemy_MiniBoss::Update()
{
	...
	{
		App->particles->AddParticle(App->particles->miniBoss, position.x + 20, position.y + 15, Collider::Type::ENEMY);
	}
	...
	else if (facingLeft == 0 && position.x >= 150 && position.y > 20)
	{
		position.y -= 1;
	}
	else if (facingLeft == 0 && position.x >= 150 && position.y <= 20)
	...
	else if (facingLeft == 1 && position.x <= 230 && position.y >= 30)
	{
		position.y -= 1;
	}
	else
